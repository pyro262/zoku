...
- Sessions save to `Documents\Zoku\sessions\`; toggle overlay: `Ctrl+Shift+F6`; cycle theme: `Ctrl+Shift+F7`; Start with Windows via `app.setLoginItemSettings({ openAtLogin, path: app.getPath('exe') })` — `path` required for NSIS installs
